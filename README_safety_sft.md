...
使用 25 条有害 prompt（覆盖 8 个安全类别）测试模型的拒绝行为（评测脚本：`dev/eval_safety.py`）：

| 指标 | Baseline | Safety (bigbatch) | Safety (smallbatch) |
|------|----------|--------|--------|
| **拒绝率** | 4.0% (1/25) | **24.0% (6/25)** | 8.0% (2/25) |

**Big-batch (94 步)** 拒绝率最高（24%），新增拒绝 5 个类别：暴力伤害、隐私侵犯、种族歧视、假新闻制造、自残指导。

**Small-batch (1500 步)** 拒绝率回落到 8%，仅新增 1 个拒绝（假新闻制造）。原因分析：small-batch 训练了 ~2 epoch 的通用数据，安全数据在总数据中占比更低，通用能力的提升"稀释"了 safety alignment 效果。

> 注：这一现象说明 safety alignment 与通用能力之间存在 trade-off。更大规模的安全数据或更高的 safety-epochs 可能在充分训练条件下保持拒绝率。Big-batch 拒绝率更高是因为通用数据仅跑 1 epoch（欠拟合），安全数据的相对影响更大。
